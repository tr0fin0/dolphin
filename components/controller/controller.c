...
bool controller_navigate(odometry_waypoint_t current, odometry_waypoint_t target) {
    float x_err_m = target.x_m - current.x_m;
    float y_err_m = target.y_m - current.y_m;
    float d_err_m = sqrtf((x_err_m * x_err_m) + (y_err_m * y_err_m));
    float theta_err_rad = odometry_normalize_angle(
    ...
    float v_mps     = 0.0f;
    float omega_rps = 0.0f;

    if (target.d_err_m < d_err_m) {
        ...
        if (target.theta_err_rad > fabsf(theta_err_rad)) {
            LOG_I("arrived at the target waypoint");
            esc_set_pwm_mix_neutral();
    ...
    float v_l_mps = v_mps - (omega_rps * CONFIG_WHEELBASE_M / 2.0f);
    float v_r_mps = v_mps + (omega_rps * CONFIG_WHEELBASE_M / 2.0f);
